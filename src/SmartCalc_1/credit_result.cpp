...
credit_result::credit_result(QWidget *parent)
    : QWidget(parent), ui(new Ui::credit_result) {
  ui->setupUi(this);

  this->createUI(QStringList()
                 << ("№ Месяца") << ("Сумма платежа, руб.")
                 << ("Платеж по основному долгу, руб.")
                 << ("Платеж по процентам, руб.") << ("Остаток долга, руб."));
  ui->tableWidget->setStyleSheet("QTableView { gridline-color: green; }");
}

credit_result::~credit_result() { delete ui; }
...
void credit_result::fill_table(double **result, int period) {
  ...
  for (int i = 0; i < period; i++) {
    ...
    ui->tableWidget->setItem(
        i, 0, new QTableWidgetItem(QString::number(result[i][0], 'g', 15)));
    ui->tableWidget->setItem(
        i, 1, new QTableWidgetItem(QString::number(result[i][1], 'g', 15)));
    ui->tableWidget->setItem(
        i, 2, new QTableWidgetItem(QString::number(result[i][2], 'g', 15)));
    ui->tableWidget->setItem(
        i, 3, new QTableWidgetItem(QString::number(result[i][3], 'g', 15)));
    ui->tableWidget->setItem(
        i, 4, new QTableWidgetItem(QString::number(result[i][4], 'g', 15)));
    full_pay += result[i][1];
