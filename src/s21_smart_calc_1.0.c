 ...
 * @return 1 - FAIL
 */
int s21_check_expression(char *expression, double value_of_x,
                         struct list **head) {
  int error_code = 0;
  ...
  for (int i = 0; !error_code; i++) {
    ...
    if (!s21_check_operand(expression[i], &i, mas_for_number, expression,
                           head)) {
      continue;
    } else if (strlen(mas_for_number)) {
      if (!s21_is_digit(mas_for_number[strlen(mas_for_number) - 1])) {
        double value = atof(mas_for_number);
        error_code = s21_check_possible_add_operand(mas_for_number, head, value,
                                                    number, 0);
        i--;
      } else if (mas_for_number[0] == 'x') {
        error_code = s21_check_possible_add_operand(mas_for_number, head,
                                                    value_of_x, x, 0);
        i--;
      }
    } else if (!s21_check_operation(expression[i], &i, head, expression)) {
      continue;
    } else if (!s21_check_parenthesis(expression[i], head,
                                      &count_parenthesis)) {
      continue;
    } else {
      error_code = 1;
 ...
 * @return 1 - FAIL
 */
int s21_check_operand(char ch, int *i, char *mas_for_number, char *expression,
                      struct list **head) {
  int error_code = 0;
...
/**
 * Проверка возможности добавления считанного числа или Х в массив и добавление
 * соответственно
 * @return SUCCESS - 0
 * @return FAIL - 1
 */
int s21_check_mas_for_number(char ch, char *mas_for_number) {
  ...
  if (length_mas && penultimate_char == X) {
    ...
    error_code = 1;
  } else if (ch == 'x' && length_mas &&
             (penultimate_char == POINT || penultimate_char == NUMBER)) {
    error_code = 1;
  } else {
...
int s21_check_func(char ch, int *i, struct list **head, char *expression) {
  ...
  if (ch == 'a') {
    if (strncmp((expression + *i), "acos", 4) == 0) {
      error_code =
          s21_check_possible_add_operand(mas_for_number, head, 0, e_acos, 5);
      *i += 3;
    } else if (strncmp((expression + *i), "asin", 4) == 0) {
      error_code =
          s21_check_possible_add_operand(mas_for_number, head, 0, e_asin, 5);
      *i += 3;
    } else if (strncmp(expression + *i, "atan", 4) == 0) {
      error_code =
          s21_check_possible_add_operand(mas_for_number, head, 0, e_atan, 5);
      *i += 3;
    } else {
      error_code = 1;
    }
    ...
    if (strncmp((expression + *i), "cos", 3) == 0) {
      error_code =
          s21_check_possible_add_operand(mas_for_number, head, 0, e_cos, 4);
      *i += 2;
    } else {
      error_code = 1;
    }
    ...
    if (strncmp((expression + *i), "ln", 2) == 0) {
      error_code =
          s21_check_possible_add_operand(mas_for_number, head, 0, e_ln, 4);
      *i += 1;
    } else if (strncmp((expression + *i), "log", 3) == 0) {
      error_code =
          s21_check_possible_add_operand(mas_for_number, head, 0, e_log, 4);
      *i += 2;
    } else {
      error_code = 1;
    }
    ...
    if (strncmp((expression + *i), "sin", 3) == 0) {
      error_code =
          s21_check_possible_add_operand(mas_for_number, head, 0, e_sin, 4);
      *i += 2;
    } else if (strncmp((expression + *i), "sqrt", 4) == 0) {
      error_code =
          s21_check_possible_add_operand(mas_for_number, head, 0, e_sqrt, 4);
      *i += 3;
    } else {
      error_code = 1;
    }
    ...
    if (strncmp((expression + *i), "tan", 3) == 0) {
      error_code =
          s21_check_possible_add_operand(mas_for_number, head, 0, e_tan, 4);
      *i += 2;
    } else {
      error_code = 1;
    }
 ...
 */
void s21_add_to_list(struct list **head, double value, enum enum_type type,
                     int priority) {
 ...
 * @return 1 - FAIL
 */
int s21_check_possible_add_operation(struct list **head, int type, int priority,
                                     char ch) {
  int error_code = 0;
  ...
  } else if ((type_last_node == -1 || type_last_node == 0) &&
             (ch == '+' || ch == '-')) {
    if (ch == '+') {
 ...
 */
void s21_create_reverse_notation_list(struct list *head,
                                      struct list **reverse_notation) {
  ...
  while (1) {
    if (help_head->type == open_bracket) {
      ...
      while (stack->type != open_bracket) {
        s21_add_to_list(reverse_notation, stack->value, stack->type,
                        stack->priority);
        s21_remove_first_node(&stack);
      }
      s21_remove_first_node(&stack);
    } else if (help_head->type == x || help_head->type == number) {
      s21_add_to_list(reverse_notation, help_head->value, help_head->type,
                      help_head->priority);
      ...
      if (help_head->priority > priority) {
        s21_push(&stack, help_head->value, help_head->type,
                 help_head->priority);
      } else {
...
void s21_calculating_an_action(struct list *tmp, struct list *buf,
                               int count_shifts, int shift_left) {
