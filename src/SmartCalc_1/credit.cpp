...
void credit::pushButton_result() {
  ...
  if (!ui->main_sum->text().contains(',') && !ui->rate->text().contains(',')) {
    ...
    if (ui->main_sum->text().length() != 0 && ui->rate->text().length() != 0 &&
        ui->lineEdit_period->text().length() != 0) {
    ...
    QMessageBox::information(this, "Наличие запятой",
                             "Введите данные через точку");
  }
}
