...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(graphics_window, &graphics::go_to_mainwindow, this,
          &MainWindow::show);
...
void MainWindow::press_button() {
  ...
  if (ui->result_label->text().endsWith("0") &&
      ui->result_label->text().size() == 1) {
...
void MainWindow::press_equal() {
  ...
  if ((ui->result_label->text().contains('x') &&
      ...
      (!ui->result_label->text().contains('x') &&
       ui->result_label->text().length() != 0)) {
    double result = 0.0;
    ...
    if (error_code == 0) {
      ...
      QMessageBox::critical(this, "inccorect",
                            "Введено неккоректное выражение");
    } else if (error_code == 3) {
      QMessageBox::critical(this, "inccorect",
                            "Выражение содержит более 255 символов");
    }
  } else {
...
void MainWindow::on_result_label_returnPressed() { press_equal(); }
